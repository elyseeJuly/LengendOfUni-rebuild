        ...
        <div className="md:col-span-1 border-r border-cyan-500/10 pr-2 space-y-2.5 max-h-72 overflow-y-auto">
          {aliens.map((alien) => (
            <button
              ...
              className={`w-full text-left p-2.5 rounded border transition-all cursor-pointer flex flex-col gap-1.5 ${
                alien.isDead ? 'border-slate-800 bg-slate-950/20 opacity-30 cursor-not-allowed' :
                selectedAlien === alien.name 
                  ? 'border-cyan-500/50 bg-cyan-950/20 text-cyan-200 shadow-[0_0_8px_rgba(6,182,212,0.15)]' 
                  : 'border-cyan-500/10 bg-slate-950/30 text-slate-400 hover:border-cyan-500/30 hover:bg-slate-900/40'
              }`}
            >
              <div className="flex items-center justify-between w-full">
                <span className="font-bold text-xs">{alien.name}</span>
                {alien.isBund && <Handshake className="w-3.5 h-3.5 text-emerald-400 shrink-0" />}
              </div>
              <div className={`text-[10px] px-1.5 py-0.5 rounded border inline-block max-w-max font-bold ${getFriendshipLabel(alien.friendship).color}`}>
