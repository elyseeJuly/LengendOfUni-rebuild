...
void CFactory::RunARound(int iWorkerCnt)
{
	...
	// 生产
	BOOL bFinish = tecTreeMgr.IsTecFinished(TT_INTERSTELLAR, _T("生产技术Ⅲ"));
	if (bFinish)
		iWeight = 5;
	else
	{
	...
	bFinish = tecTreeMgr.IsTecFinished(TT_INFORMATION, _T("质能转换")); 
...
void CFactory::RunARoundNormal()
{
	CEarthCivilization &earthCivi = ((CLengendOfUniApp*)AfxGetApp())->m_game->GetEarthCivilization();
	CTecTreeManager &tecTreeMgr = earthCivi.GetTecTreeManager();
	BOOL bFinish = tecTreeMgr.IsTecFinished(TT_MILITARY, _T("行星建设Ⅲ"));
	if (bFinish)
		SetBuildPerRound(50);
	else
	{
		bFinish = tecTreeMgr.IsTecFinished(TT_MILITARY, _T("行星建设Ⅱ"));
		if (bFinish)
			SetBuildPerRound(40);
		else
		{
