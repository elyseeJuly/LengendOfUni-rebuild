...
void CStope::RunARoundNormal()
{
	...
	CEarthCivilization &earthCivi = ((CLengendOfUniApp*)AfxGetApp())->m_game->GetEarthCivilization();
	CTecTreeManager &tecTreeMgr = earthCivi.GetTecTreeManager();
	BOOL bFinish = tecTreeMgr.IsTecFinished(TT_MILITARY, sTec + _T("Ⅲ"));
	if (bFinish)
		SetBuildPerRound(50);
	else
	{
		bFinish = tecTreeMgr.IsTecFinished(TT_MILITARY, sTec + _T("Ⅱ"));
		if (bFinish)
			SetBuildPerRound(40);
		else
		{
			bFinish = tecTreeMgr.IsTecFinished(TT_MILITARY, sTec + _T("Ⅰ"));
			if (bFinish)
				SetBuildPerRound(20);
		}
	}
