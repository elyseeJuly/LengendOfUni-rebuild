...
// YEAR_STRING removed — epoch name is now dynamic, see UpdateYearLabel()
...
BOOL CLengendOfUniDlg::OnInitDialog()
{
	...
	CIniConfig iniConfig(_T("ui_theme.ini"));
	CString sTheme = iniConfig.GetString(_T("UI"), _T("Theme"));
	if (sTheme.CompareNoCase(_T("Light")) == 0)
	{
		m_bLightTheme = TRUE;
...
void CLengendOfUniDlg::OnBnClickedButtonPlanFinish()
{
	...
	if (tecTreeMgr.IsTecFinished(TT_AEROSPACE, _T("行星发动机Ⅲ型")) && 
	    tecTreeMgr.IsTecFinished(TT_INTERSTELLAR, _T("新家园选址")))
	{
		CHelper::PersonMsgBox(NULL, _T("流浪地球计划成功执行！地球驶离太阳系，人类赢得了流浪胜利！\n（游戏结束）"));
	...
	// 2. 数字永生胜利
	if (tecTreeMgr.IsTecFinished(TT_INFORMATION, _T("数字方舟")))
	{
		CHelper::PersonMsgBox(NULL, _T("人类完成了意识上传，在数字世界中获得了永生。我们迎来了数字永生胜利！\n（游戏结束）"));
		((CLengendOfUniApp*)AfxGetApp())->m_game->GetGameEventManager().RunNormalEvent(_T("end"), ET_STRINGINDEX);
		CResultDlg dlg;
		dlg.DoModal();
		CDialog::OnCancel();
	...
	// 3. 威慑胜利 (存活到200年且有执剑人)
	int iYear = ((CLengendOfUniApp*)AfxGetApp())->m_game->GetYear();
	if (iYear >= 200 && !earthCivi.GetSwordholder().IsEmpty())
	{
		CHelper::PersonMsgBox(NULL, _T("长达200年的威慑纪元降临，地球与外星文明建立起了恐怖平衡。人类赢得了威慑胜利！\n（游戏结束）"));
		((CLengendOfUniApp*)AfxGetApp())->m_game->GetGameEventManager().RunNormalEvent(_T("end"), ET_STRINGINDEX);
		CResultDlg dlg;
		dlg.DoModal();
		CDialog::OnCancel();
	...
	{
		CHelper::PersonMsgBox(NULL, _T("地球舰队所向披靡，所有敌对文明均已被征服。人类赢得了征服胜利！\n（游戏结束）"));
		((CLengendOfUniApp*)AfxGetApp())->m_game->GetGameEventManager().RunNormalEvent(_T("end"), ET_STRINGINDEX);
		CResultDlg dlg;
		dlg.DoModal();
		CDialog::OnCancel();
	...
	iYear = ((CLengendOfUniApp*)AfxGetApp())->m_game->GetYear();
	CString sYear;
	...
	CTecTreeManager &tecTreeMgr2 = earthCivi.GetTecTreeManager();
	BOOL bFinish = tecTreeMgr2.IsTecFinished(TT_PHYSICS, _T("50光年远镜"));
	if (bFinish)
		GetDlgItem(IDC_BUTTON_STARSYS2)->EnableWindow(TRUE);
	bFinish = tecTreeMgr2.IsTecFinished(TT_PHYSICS, _T("1万光年远镜"));
	if (bFinish)
		GetDlgItem(IDC_BUTTON_STARSYS3)->EnableWindow(TRUE);
	bFinish = tecTreeMgr2.IsTecFinished(TT_PHYSICS, _T("银河系远镜"));
	if (bFinish)
...
{
	int iYear = ((CLengendOfUniApp*)AfxGetApp())->m_game->GetYear();
	EPOCH_TYPE epoch = ((CLengendOfUniApp*)AfxGetApp())->m_game->GetEpoch();
	CString epochNames[] = {_T("危机纪元"), _T("威慑纪元"), _T("广播纪元"), _T("掩体纪元"), _T("银河纪元")};
	m_sYear.Format(_T("%s %d 年"), epochNames[(int)epoch], iYear);
	UpdateData(FALSE);
...
void CLengendOfUniDlg::InitOthers()
{
	...
	// 星系切换按钮，看是否enable
	CTecTreeManager &tecTreeMgr = earthCivi.GetTecTreeManager();
	BOOL bFinish = tecTreeMgr.IsTecFinished(TT_PHYSICS, _T("50光年远镜"));
	if (bFinish)
	{
		m_canReachStarArea = SA_50LIGHTYEAR;
		GetDlgItem(IDC_BUTTON_STARSYS2)->EnableWindow(TRUE);
	}
	bFinish = tecTreeMgr.IsTecFinished(TT_PHYSICS, _T("1万光年远镜"));
	if (bFinish)
	{
		m_canReachStarArea = SA_1WLIGHTYEAR;
		GetDlgItem(IDC_BUTTON_STARSYS3)->EnableWindow(TRUE);
	}
	bFinish = tecTreeMgr.IsTecFinished(TT_PHYSICS, _T("银河系远镜"));
	if (bFinish)
	{
