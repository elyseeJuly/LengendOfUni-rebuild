...
//	std::list<CString>			m_lstMyPerson;
	...
	std::vector<CString>		m_vecWallfacers;
	CString						m_sSwordholder;

public:
	void AddWallfacer(CString name);
	void RemoveWallfacer(CString name);
	BOOL IsWallfacer(CString name);
	void SetSwordholder(CString name);
	CString GetSwordholder();
	virtual void SetPopulation(int iVal);
