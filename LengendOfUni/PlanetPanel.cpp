...
void CPlanetPanel::InitNonAlienBtns()
{
	...
	{
		m_sFactory.Format(FACTORY_STRING, _T("无"));
	...
	else
	{
		...
		if (iCrtBuild == iTtlBuild)	// 已建
		{
			m_sFactory.Format(FACTORY_STRING, _T("已建"));
			GetDlgItem(IDC_BUTTON_FACTORY)->ShowWindow(SW_HIDE);
		}
		else	// 建造中
		{
			CString sFormat;
			sFormat.Format(_T("建造中 %d/%d"), iCrtBuild, iTtlBuild);
			m_sFactory.Format(FACTORY_STRING, sFormat);
			GetDlgItem(IDC_BUTTON_FACTORY)->ShowWindow(SW_HIDE);
		}
	}
	...
	if (m_bPlanet)
	{
		bFinish = tecTreeMgr.IsTecFinished(TT_SPACEFIGHT, _T("行星开发Ⅰ"));
		if (bFinish)
	...
	{
		 bFinish = tecTreeMgr.IsTecFinished(TT_SPACEFIGHT, _T("恒星开发Ⅰ"));
		if (bFinish)
			GetDlgItem(IDC_BUTTON_STOPE)->EnableWindow(TRUE);
		else
			GetDlgItem(IDC_BUTTON_STOPE)->EnableWindow(FALSE);
