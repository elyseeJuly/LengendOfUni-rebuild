...
void CCity::RunARound()
{
	...
	BOOL bFinish = tecTreeMgr.IsTecFinished(TT_SPACEFIGHT, _T("行星殖民Ⅲ"));
	if (bFinish)
		SetBuildPerRound(50);
	else
	{
		bFinish = tecTreeMgr.IsTecFinished(TT_SPACEFIGHT, _T("行星殖民Ⅱ"));
		if (bFinish)
			SetBuildPerRound(40);
		else
		{
			bFinish = tecTreeMgr.IsTecFinished(TT_SPACEFIGHT, _T("行星殖民Ⅰ"));
			if (bFinish)
				SetBuildPerRound(20);
		}
	}
	...
	int iWeight = 2;
	bFinish = tecTreeMgr.IsTecFinished(TT_CULTURETEC, _T("人口Ⅲ"));
	if (bFinish)
		iWeight = 5;
	else
	{
		bFinish = tecTreeMgr.IsTecFinished(TT_CULTURETEC, _T("人口Ⅱ"));
		if (bFinish)
			iWeight = 4;
		else
		{
			bFinish = tecTreeMgr.IsTecFinished(TT_CULTURETEC, _T("人口Ⅰ"));
			if (bFinish)
				iWeight = 3;
		}
	}
