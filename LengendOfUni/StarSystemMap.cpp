...
void CStarSystemMap::OnStarBtnClick(UINT uID)
{
	...
	BOOL bFinish = tecTreeMgr.IsTecFinished(TT_SPACEFIGHT,sTec);
	if (!bFinish)
	{
		CString sInfo;
	...
	for (int i = 0; i < m_vecButtons.size(); i ++)
	{
		...
		if (pStar)
		{
			...
			if (m_starArea == SA_50LIGHTYEAR)
				bFinish = tecTreeMgr.IsTecFinished(TT_ASTROPHYSICS, _T("太阳电波放大(50光年)"));
			else if (m_starArea == SA_1WLIGHTYEAR)
				bFinish = tecTreeMgr.IsTecFinished(TT_ASTROPHYSICS, _T("太阳电波放大(1万光年)"));
			else if (m_starArea == SA_GALAXY)
				bFinish = tecTreeMgr.IsTecFinished(TT_ASTROPHYSICS, _T("太阳电波放大(银河系)"));
			if (!bFinish)
			{
